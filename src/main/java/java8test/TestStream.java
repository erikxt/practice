...
public class TestStream {
    ...
    private static boolean isPrime(long n) {
        return n > 1 && IntStream.rangeClosed(2, (int) Math.sqrt(n)).noneMatch(divisor -> n %
                divisor == 0);
    }
    ...
    public static void main(String[] args) {
        ...
        IntStream.range(0, 5_600_000_00).filter(p -> p % 2 == 0).toArray();

        long t1 = System.nanoTime();

        //和上面功能一样，这里是用并行流来计算

        IntStream.range(0, 5_600_000_00).parallel().filter(p -> p % 2 == 0).toArray();
